    ...
    if geom:
        ...
        try:
            ...
            if loc:
                ...
                # Si tenemos calle, y town, combinamos. Sino fallback a town o municipio de ARCGIS
                if road and town:
                    return f"{road}, {town}"
                if town:
                    return town
            ...
            pass
    # fallback a municipio de ArcGIS si no hay geolocalización o calle
    return a.get("MUN_NOM", "ubicació desconeguda")

def tweet_body(a, place, titulo=""):
    hora = datetime.fromtimestamp(a["ACT_DAT_ACTUACIO"]/1000, tz=timezone.utc)\
           .astimezone(ZoneInfo("Europe/Madrid")).strftime("%H:%M")
    return (f"{titulo}\n🔥 Incendi {classify(a)} a {place}\n"
...
def main():
    ...
    for idx, f in enumerate(tweets):
        a = f["attributes"]
        titulo = "Actuació més recent" if idx == 0 else "Incendi actiu més rellevant"
        body = tweet_body(a, place_from_geom(a, f.get("geometry")), titulo)
        send(body, api)
